...
spec = do
  describe "Group Parser" do
    ...
    it "parses f(T)" do
      let state = runParser parseExprGroup "sin(1)"
      state `shouldEqual` Right { rest: ""
                                , token: [ ExprFunc Defaults.fsin
                                         , ExprOpenParen
                                         , ExprLiteral 1.0
                                         , ExprCloseParen
                                         ]
    ...
    it "parses f(T, T)" do
      let state = runParser parseExprGroup "max(1, 3)"
      state `shouldEqual` Right { rest: ""
                                , token: [ ExprFunc Defaults.fmax
                                         , ExprOpenParen
                                         , ExprLiteral 1.0
                                         , ExprComma
                                         , ExprLiteral 3.0
...
hasNumbers :: Func
hasNumbers = Func { symbol: "hasNumbers123"
                  , arity: 1
                  , exec: \_ -> Just 0.0
                  }
