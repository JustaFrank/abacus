...
execFunc f arity stack =
  let
    { before: top, after: rem } = splitAt arity stack
  in
    (_ : rem) <<< ExprLiteral <$> (f =<< traverse getLiteral (reverse top))
...
pushOper s@{ output, opers } oper = do
  ...
  if criteria oper head then
    pushOper (s { output = head : output, opers = tail }) oper
  else
    Just $ s { opers = (ExprOper oper) : opers }

popUntilParen :: ShuntingS -> Maybe ShuntingS
popUntilParen s@{ output, opers } = do
  { head, tail } <- uncons opers
  if head == ExprOpenParen then
    Just $ s { opers = tail }
  else
    popUntilParen (s { output = head : output, opers = tail })

criteria :: Oper -> ExprToken -> Boolean
criteria _ (ExprFunc _) = true

criteria (Oper oper) (ExprOper (Oper { assoc: RightAssoc, preced })) = preced > oper.preced
